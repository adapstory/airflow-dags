...
@pytest.mark.parametrize("server_version", ("3.3.0", "3.4.1"))
def test_history_client_uses_one_short_lived_dedicated_jwt_and_complete_v2_pagination(
    ...
    responses: list[dict[str, Any]] = [
        ...
        {"git_version": f"airflow-{server_version}", "version": server_version},
        {
...
def test_history_client_never_reuses_a_jwt_across_observations(tmp_path: Path) -> None:
    ...
    def transport(**request: Any) -> dict[str, Any]:
        ...
        if path == "/api/v2/version":
            return {"git_version": "airflow-3.3.0", "version": "3.3.0"}
...
@pytest.mark.parametrize("server_version", ("2.11.2", "4.0.0", "3.3", "3.3.0rc1"))
def test_history_client_rejects_unsupported_airflow_server_versions(
    tmp_path: Path,
    server_version: str,
) -> None:
    jwt = _jwt(
...
def test_kubernetes_fence_requires_a_predeclared_config_map() -> None:
    ...
    with pytest.raises(ValueError, match="predeclared"):
        client.acquire(parent_airflow_run=_parent_run())
    assert api.patches == []
...
def test_manual_d19_is_blocked_only_while_a_real_fence_is_active() -> None:
    ...
    with pytest.raises(ValueError, match="predeclared"):
        missing.assert_unfenced_run_allowed()
...
@pytest.mark.parametrize(
    "run_id",
    (
        "d19_canary__" + "a" * 32 + "__01__attempt_000038",
        "d19_governance__" + "a" * 32 + "__01__attempt_000038",
        "d19_governance__" + "a" * 32 + "__03__attempt_000038",
    ),
)
def test_d19_admission_accepts_only_canonical_authenticated_bootstrap_attempts(
    run_id: str,
) -> None:
    bootstrap_run = {
        **_d19_airflow_run(),
    ...
    admitted = admit_d19_run(
        dag_run_conf={"generated_at": "2026-07-17T00:00:00Z"},
        airflow_run=bootstrap_run,
        fence_client=_AdmissionFenceClient(active=None),
    )

    assert admitted["airflowRun"] == bootstrap_run

    for malformed_run_id, run_type in (
        ("d19_canary__" + "a" * 31 + "__01__attempt_000038", "manual"),
        ("d19_canary__" + "a" * 32 + "__02__attempt_000038", "manual"),
        ("d19_governance__" + "a" * 32 + "__04__attempt_000038", "manual"),
        ("d19_canary__" + "a" * 32 + "__01__attempt_000000", "manual"),
        ("d19_canary__" + "a" * 32 + "__01__attempt_000038", "operator_triggered"),
    ):
